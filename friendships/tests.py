...
class FriendshipApiTests(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(2):
            ...
            Friendships.objects.create(from_user=follower, to_user=self.user1)
        for i in range(3):
            following = self.create_user(f'user1_following{i}')
            Friendships.objects.create(from_user=self.user1, to_user=following)
    ...
    def test_unfollow(self):
        ...
        # Normal unfollow
        Friendships.objects.create(from_user=self.user1, to_user=self.user2)
        count = Friendships.objects.count()
        response = self.user1_client.post(url)
